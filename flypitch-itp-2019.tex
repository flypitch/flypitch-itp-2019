...
H_left_left : Γ ≤ d,
H_left_right : Γ ≤ e,
H_right_right_right : Γ ≤ c,
H_right_left_left : Γ ≤ f,
...
\end{lstlisting}
\end{definition}
That is, we send each \lil{c : 𝒞} all the subsets which satisfy the specification given by \lil{c}. This is a clopen set, hence regular.

Crucially, this embedding is \emph{dense}:
\begin{lstlisting}
lemma 𝒞_dense {b : 𝔹} (H : ⊥ < b) : ∃ p : 𝒞, ι p ≤ b  
\end{lstlisting}
Recalling that $\leq$ in $\B$ is subset-inclusion, we see that this is essentially because the image of $\iota : \mathcal{C} \to \B$ \emph{is} is the standard basis for the product topology. Our chosen encoding of the Cohen poset also made it easier to perform this identification when formalizing the proof.
...
\subsection{Representing Lean's ordinals inside \lil{pSet}}
TODO(jesse) emphasize how Lean's ordinals play the key role, rather than the ordinals inside pSet.
\subsection{Adding $\aleph_2$-many distinct Cohen reals}
As we saw in \autoref{def-powerset}, for any $\B$-valued set $x$, characteristic functions into $\B$ from the underlying type of $x$ determine $\B$-valued subsets of $x$. While the ingredients $\aleph_2$ and $\mathbb{N}$ for $\B$ are external to \lil{bSet 𝔹}, they are represented nonetheless inside \lil{bSet 𝔹} by their check-names (\autoref{def-check}) $\check{\aleph_2}$ and $\check{\mathbb{N}}$.
